...
import { HeartIcon, CalendarIcon, MapPinIcon, ClockIcon, SparklesIcon, UsersIcon } from '@heroicons/react/24/outline';
import { useEventHero } from '@/hooks/useEventHero';
...
export const EventHero = ({ eventType = 'wedding' }: EventHeroProps) => {
  const { settings, isLoading, error, eventType: currentEventType, isCompatibilityMode } = useEventHero(eventType);
    ...
    return (
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center">
          ...
          <p className="mt-4 text-lg text-muted-foreground">Memuat data acara...</p>
        </div>
      </section>
    );
  }
  ...
  if (error || !settings) {
    return (
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-primary/20 via-rose-gold/15 to-accent/20" />
        <div className="relative z-10 text-center">
          <SparklesIcon className="h-20 w-20 text-muted-foreground mx-auto mb-4" />
          <p className="text-lg text-muted-foreground">
            {error || 'Data acara tidak ditemukan'}
          </p>
          {isCompatibilityMode && (
            <p className="text-sm text-muted-foreground/70 mt-2">
              Mode kompatibilitas: {currentEventType}
            </p>
          )}
        </div>
      </section>
  ...
  const eventDate = new Date(`${settings.event_date}T${settings.event_time}`);
  const formattedDate = format(eventDate, 'd MMMM yyyy', { locale: id });
  const formattedTime = format(eventDate, 'HH:mm', { locale: id });
  ...
  // Get display icon based on event type
  const getEventIcon = () => {
  ...
  // Get event title based on participants
  const getEventTitle = () => {
    const participants = settings.primary_participants;
    if (participants.length === 2 && currentEventType === 'wedding') {
      return participants; // For wedding: [groom_name, bride_name]
    }
    return participants; // For other events: flexible participant names
  };

  const participantNames = getEventTitle();
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        ...
        {settings.hero_content.background_image && (
          <img 
            src={settings.hero_content.background_image} 
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="floating mb-12">
            <div className="relative">
              <EventIcon className="h-20 w-20 text-primary mx-auto mb-8 drop-shadow-lg" />
              <div className="absolute inset-0 h-20 w-20 mx-auto animate-ping">
                <EventIcon className="h-20 w-20 text-primary/30" />
              </div>
            </div>
          </div>
          ...
          <div className="mb-12">
            {currentEventType === 'wedding' && participantNames.length === 2 ? (
              ...
              <h1 className="text-6xl md:text-8xl font-bold mb-8 fade-in-up">
                <span className="text-gradient block hover:scale-105 transition-transform duration-300">
                  {participantNames.length > 0 ? participantNames.join(' & ') : 'Event'}
                </span>
              </h1>
            )}
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-12 fade-in-up delay-300">
            ...
            <div className="elegant-card bg-card/90 backdrop-blur-md rounded-2xl p-8 border border-primary/20 group hover:border-primary/40 transition-all duration-300">
              <div className="flex items-center justify-center gap-4 mb-6">
                ...
                <div className="text-left">
                  <span className="text-sm font-semibold text-primary block">Tanggal Acara</span>
              ...
              <div className="flex items-center justify-center gap-2 text-muted-foreground">
                ...
                <span className="text-lg">{format(eventDate, 'EEEE', { locale: id })}, {formattedTime} WIB</span>
              </div>
            </div>
            ...
            <div className="elegant-card bg-card/90 backdrop-blur-md rounded-2xl p-8 border border-rose-gold/20 group hover:border-rose-gold/40 transition-all duration-300">
              ...
              <p className="text-xl font-bold text-rose-gold mb-2">
                {settings.primary_venue.name}
              </p>
              <p className="text-muted-foreground">
                {settings.primary_venue.address}
              </p>
              {settings.secondary_venue && (
                <div className="mt-4 pt-4 border-t border-rose-gold/20">
                  <p className="text-lg font-semibold text-rose-gold mb-1">
                    {settings.secondary_venue.name}
                  </p>
                  <p className="text-sm text-muted-foreground">
                    {settings.secondary_venue.address}
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="flex flex-col sm:flex-row gap-6 justify-center items-center fade-in-up delay-400 mb-16">
            <Button variant="premium" size="xl" className="min-w-56 group relative overflow-hidden">
              <span className="relative z-10 flex items-center gap-3">
                <EventIcon className="h-5 w-5 group-hover:scale-110 transition-transform" />
          ...
          {/* Event Countdown */}
          {settings.countdown_enabled && timeUntilEvent > 0 && (
          ...
          {isCompatibilityMode && (
            <div className="fixed top-4 right-4 bg-accent/80 backdrop-blur-sm text-accent-foreground px-3 py-1 rounded-full text-sm font-medium shadow-lg z-50">
              Plugin: {currentEventType} (v1.0)
            </div>
          )}
